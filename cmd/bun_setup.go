...
func newBunSetupCmd() *cobra.Command {
	var (
		flagMantine      bool
		flagNoTailwind   bool
	...
	cmd := &cobra.Command{
		Use:   "new-bun [name]",
		Short: "Create a new React app powered by Bun",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := installer.ScaffoldBunProject(); err != nil {
				return err
			}

			if err := installer.CleanupBunScaffold(); err != nil {
				return err
			}
			...
			if err := runner.Run("bun", "dev"); err != nil {
				return err
			}
			return nil
		},
...
func newBunSetupAliasCmd() *cobra.Command {
	cmd := newBunSetupCmd()
	cmd.Use = "bun-setup [name]"
	cmd.Short = "Deprecated: use new-bun to create a Bun project"
	cmd.Deprecated = "use new-bun instead"

	run := cmd.RunE
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		logger.Warning("`bun-setup` is deprecated; use `new-bun` instead.")
		if run != nil {
			return run(cmd, args)
		}
		return nil
